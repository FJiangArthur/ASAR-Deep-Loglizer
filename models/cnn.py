...
class CNN(tf_BasedModel):
    def __init__(
        ...
        self.conv2a = tf.keras.layers.Conv2D(filters=filters1,
                                             kernel_size=kernel1,
                                             input_shape=(None, None, self.embedding_dim, 1)
                                             )
        self.bn2a = tf.keras.layers.BatchNormalization()

        self.conv2b = tf.keras.layers.Conv2D(filters=filters2, kernel_size=kernel2, padding='same')
        self.bn2b = tf.keras.layers.BatchNormalization()

        self.conv2c = tf.keras.layers.Conv2D(filters=filters3, kernel_size=kernel3)
        self.bn2c = tf.keras.layers.BatchNormalization()

        self.linear = tf.keras.layers.Dense(self.num_labels, activation = None)
