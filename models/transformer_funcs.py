...
def Attention_Matrix(K, Q, use_mask=False):
    ...
    matrix = tf.math.divide(tf.matmul(Q, tf.transpose(K, perm=[0,2,1])), math.sqrt(K.shape[2]))
    if use_mask:
        matrix = matrix+atten_mask
...
class Atten_Head(tf.keras.layers.Layer):
    def __init__(self, input_size, output_size, use_mask):
        ...
        self.K = self.add_weight(
            shape=(input_size, output_size), initializer="random_normal", trainable=True
        )
    ...
    def call(self, inputs_for_keys, inputs_for_values, inputs_for_queries):
        ...
        Q = tf.tensordot(inputs_for_values, self.Q, axes=[[2],[0]])
        att = Attention_Matrix(K, Q, use_mask=self.use_mask)
        out = tf.matmul(att, V)
        return out
